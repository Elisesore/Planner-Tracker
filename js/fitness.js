...
document.addEventListener('DOMContentLoaded', () => {
   ...
   const currentDayElement = document.querySelector(`.day-item[data-day="${currentDay}"]`);
   ...
   modalBody.innerHTML = `
       <div class="weight-inputs">
           <div class="input-group">
               ...
               <input type="number" id="startingWeight" placeholder="Enter starting weight" value="${fitnessData.weight.starting || ''}">
           </div>
           <div class="input-group">
...
function saveWeight() {
    const starting = parseFloat(document.getElementById('startingWeight').value);
    ...
    if (starting && starting > 0) {
        fitnessData.weight.starting = starting;
    }
...
function setupEventListeners() {
   ...
   if (stepsCard) {
       stepsCard.addEventListener('click', openStepsModal);
   }
   
   if (waterCard) {
       waterCard.addEventListener('click', openWaterModal);
   }
